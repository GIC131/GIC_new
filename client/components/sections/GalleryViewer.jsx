...
import { Plus, X, Download, Trash2, LogOut, Video } from "lucide-react";
...
const GalleryViewer = () => {
  ...
  const deleteImage = async (imageId) => {
    try {
      const response = await fetch(`${serverUrl}/api/gallery/${imageId}`, {
        method: 'DELETE',
      });
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <div>
            ...
            <p className="text-gray-600 dark:text-gray-400">
              {isAuthenticated ? `Manage your media collection (${images.length} files)` : `Browse our media collection (${images.length} files)`}
            </p>
          </div>
          ...
          <Card className="text-center py-20">
            <CardContent>
              ...
              <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">
                {isAuthenticated ? 'No media yet' : 'No media available'}
              </h3>
              <p className="text-gray-600 dark:text-gray-400 mb-4">
                {isAuthenticated 
                  ? 'Upload your first image or video to get started'
                  : 'Check back later for new media'
                }
              </p>
              {isAuthenticated && (
                <label className="cursor-pointer">
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="relative max-w-4xl max-h-[90vh] bg-white dark:bg-gray-900 rounded-lg overflow-hidden">
            ...
            {selectedImage.type === 'video' ? (
              <video
                src={`${serverUrl}${selectedImage.path}`}
              ...
              <img
                src={`${serverUrl}${selectedImage.path}`}
                alt={selectedImage.originalName || selectedImage.name}
                className="w-full h-full object-contain max-h-[80vh]"
              />
            )}
